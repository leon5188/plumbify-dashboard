...
export default function DashboardView({ data, loading, error }: DashboardViewProps) {
  // Mock data for technician leaderboard (fallback)
  ...
  // Mock data for active jobs table (fallback)
  ...
  const techniciansList = data?.leaderboard || fallbackTechnicians;
  const activeJobsList = data?.activeJobs || fallbackActiveJobs;
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="bg-red-950/20 border border-red-800 text-red-400 p-4 rounded-xl text-sm">
          ⚠️ GHL Connection Error: {error}. Displaying dashboard placeholder data.
        </div>
      )}
              ...
              <div>
                <div className="flex justify-between text-xs font-bold mb-1">
                  ...
                  <span className="text-orange-glow font-bold">
                    {loading ? 'Syncing...' : (data?.workflows?.total || 0)}
                  </span>
                </div>
                {data?.workflows && (
